...
export function getMatchCaptures(
  matches: Parser.QueryMatch[],
  name: string
): Parser.SyntaxNode[] {
  return matches
    .map(({ captures }) => getCapture(captures, name))
    .filter((c) => c !== null);
}
 ...
 */
export function findBySameNode(
  root: Parser.SyntaxNode,
  ...
  return [...findByPath(root, path), ...findByReference(root, label)].sort(
 ...
 */
export function getContainingNode(
...
export function getProperty(
  ...
  const childProps = props.filter((prop) =>
    getContainingNode(prop)?.equals(node)
  );
...
function expectType(node: Parser.SyntaxNode, ...types: string[]) {
  if (!types.includes(node.type)) {
    throw new ParseError(
      node,
      `Expected ${types.join(" | ")} but got ${node.type}`
    );
  }
}
...
export function parsePhandleArray(
  node: Parser.SyntaxNode | null
): Parser.SyntaxNode[] {
